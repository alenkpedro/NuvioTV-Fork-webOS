...
export function installSeek({video,timeline,update,reveal,thumbnails}) {
  ...
  function cancel(){thumbnails?.cancel();preview=null;direction=null;update();}
  function commit(){if(preview===null)return;thumbnails?.commit();const target=preview;preview=null;direction=null;if(eligible())video.currentTime=clamp(target);update();}
  function key(key,event){if(!['ArrowLeft','ArrowRight'].includes(key)||!eligible())return false;
    ...
    preview=clamp((preview??video.currentTime)+(key==='ArrowLeft'?-step:step));reveal();update();thumbnails?.preview(preview);event.preventDefault();return true;}
  ...
  function input(){if(eligible()){preview=clamp(Number(timeline.value));reveal();update();thumbnails?.preview(preview);}}
