...
export function installThumbnails({screen,video,context,settings,el,blocked,onUnavailable}) {
 ...
 let disposed=false,protectedMedia=false,failures=0,unavailable='',lastCapture=-Infinity,timer,shown=null;
 ...
 function capture(){
  ...
  const now=performance.now(),time=video.currentTime,bucket=Math.floor(time/10);if(now-lastCapture<captureWindow()||frames.some(f=>f.bucket===bucket))return;lastCapture=now;
  ...
  try{canvas.getContext('2d',{alpha:false}).drawImage(video,0,0,canvas.width,canvas.height);frames.push({bucket,time,canvas});failures=0;if(frames.length>64){const old=frames.shift();old.canvas.width=old.canvas.height=0;}}catch{canvas.width=canvas.height=0;failures++;if(failures===1){unavailable='A TV não permitiu capturar o quadro desta fonte (a origem não autoriza leitura); as miniaturas ficam desligadas nesta reprodução.';onUnavailable?.(unavailable);hide();}}
 }
 ...
 const observer=new MutationObserver(()=>{if(blocked())hide();});if(enabled){observer.observe(screen,{childList:true});video.addEventListener('timeupdate',capture);video.addEventListener('seeked',capture);video.addEventListener('encrypted',encrypted);document.addEventListener('visibilitychange',visibility);window.addEventListener('blur',hide);}
