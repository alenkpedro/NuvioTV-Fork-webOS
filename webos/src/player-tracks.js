...
export function installTrackControls({ screen, video, context, addons, settings, memoryState = {}, persist, el, button, onOpen, onClose }) {
  ...
  function draw() {
    ...
    const panel = el('section', { class: `track-panel${panelKind === 'subtitles' && !editor ? ' subtitle-filter-panel' : ''}` }, el('div', { class: 'track-heading' }, el('h2', {}, panelKind === 'audio' ? 'Áudio' : 'Legendas'), button('Fechar', close, { class: 'track-close', 'data-dismiss': true, 'data-track-key': 'close' })));
    ...
    if (preferences.rememberTracks && (remembered.audio || remembered.subtitles)) panel.append(row('Usar idiomas dos ajustes', 'Esquecer escolhas deste título neste perfil', resetRemembered, 'forget-tracks'));
