...
if (!/^[\w-]+\/[\w.-]+$/.test(repo ?? '') || !/^[\w.-]+$/.test(tag ?? '')) throw Error('Usage: node scripts/generate-homebrew.mjs owner/repo webos-v0.14.0');
...
const manifest = {
  ...
  appDescription: 'Netflix Sans nas legendas externas, velocidade, atraso por episódio e confirmação Ainda assistindo.',
...
const index = { paging: { page: 1, count: 1, maxPage: 1, itemsTotal: 1 }, packages: [{ id: app.id, title: app.title, iconUri: manifest.iconUri, manifest, pool: 'main', shortDescription: 'Player e legendas Netflix Sans para LG webOS 24.' }] };
