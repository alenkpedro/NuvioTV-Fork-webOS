...
if (!/^[\w-]+\/[\w.-]+$/.test(repo ?? '') || !/^[\w.-]+$/.test(tag ?? '')) throw Error('Usage: node scripts/generate-homebrew.mjs owner/repo webos-v0.9.0');
...
const manifest = {
  ...
  appDescription: 'Descobrir com filtros, busca ao digitar, pesquisas recentes e organização dos catálogos por perfil.',
...
const index = { paging: { page: 1, count: 1, maxPage: 1, itemsTotal: 1 }, packages: [{ id: app.id, title: app.title, iconUri: manifest.iconUri, manifest, pool: 'main', shortDescription: 'Descobrir, busca e sincronização Nuvio para LG webOS 24.' }] };
