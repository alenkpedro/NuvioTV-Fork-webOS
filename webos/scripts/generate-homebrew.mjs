...
if (!/^[\w-]+\/[\w.-]+$/.test(repo ?? '') || !/^[\w.-]+$/.test(tag ?? '')) throw Error('Usage: node scripts/generate-homebrew.mjs owner/repo webos-v0.15.1');
...
const manifest = {
  ...
  appDescription: 'Netflix Sans nas legendas externas e internas de texto acessível, com prévia e verificação da fonte.',
...
const index = { paging: { page: 1, count: 1, maxPage: 1, itemsTotal: 1 }, packages: [{ id: app.id, title: app.title, iconUri: manifest.iconUri, manifest, pool: 'main', shortDescription: 'Correção da fonte das legendas para LG webOS 24.' }] };
