...
if (!/^[\w-]+\/[\w.-]+$/.test(repo ?? '') || !/^[\w.-]+$/.test(tag ?? '')) throw Error('Usage: node scripts/generate-homebrew.mjs owner/repo webos-v0.6.0');
...
const manifest = {
  ...
  appDescription: 'Perfis Nuvio com PIN, addons por perfil e leitura da biblioteca da conta.',
...
const index = { paging: { page: 1, count: 1, maxPage: 1, itemsTotal: 1 }, packages: [{ id: app.id, title: app.title, iconUri: manifest.iconUri, manifest, pool: 'main', shortDescription: 'Perfis, biblioteca da conta e player para LG webOS 24.' }] };
