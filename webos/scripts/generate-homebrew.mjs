...
if (!/^[\w-]+\/[\w.-]+$/.test(repo ?? '') || !/^[\w.-]+$/.test(tag ?? '')) throw Error('Usage: node scripts/generate-homebrew.mjs owner/repo webos-v0.3.0');
...
const manifest = {
  ...
  appDescription: 'Login Nuvio por QR code e importação dos addons do perfil principal. Atualização para LG webOS.',
...
const index = { paging: { page: 1, count: 1, maxPage: 1, itemsTotal: 1 }, packages: [{ id: app.id, title: app.title, iconUri: manifest.iconUri, manifest, pool: 'main', shortDescription: 'Login Nuvio, addons da conta e layout do fork para LG webOS 24.' }] };
