...
if (!/^[\w-]+\/[\w.-]+$/.test(repo ?? '') || !/^[\w.-]+$/.test(tag ?? '')) throw Error('Usage: node scripts/generate-homebrew.mjs owner/repo webos-v0.18.0');
...
const manifest = {
  ...
  appDescription: 'Tela de pausa com sinopse e elenco, logos dos títulos e player adaptado do fork.',
...
const index = { paging: { page: 1, count: 1, maxPage: 1, itemsTotal: 1 }, packages: [{ id: app.id, title: app.title, iconUri: manifest.iconUri, manifest, pool: 'main', shortDescription: 'Player do fork com logos e tela de pausa para LG webOS 24.' }] };
