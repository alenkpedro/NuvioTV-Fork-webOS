...
if (!/^[\w-]+\/[\w.-]+$/.test(repo ?? '') || !/^[\w.-]+$/.test(tag ?? '')) throw Error('Usage: node scripts/generate-homebrew.mjs owner/repo webos-v0.7.0');
...
const manifest = {
  ...
  appDescription: 'Histórico Nuvio por perfil, retomada de filmes e episódios e resolução de conflitos.',
...
const index = { paging: { page: 1, count: 1, maxPage: 1, itemsTotal: 1 }, packages: [{ id: app.id, title: app.title, iconUri: manifest.iconUri, manifest, pool: 'main', shortDescription: 'Perfis, histórico da conta e player para LG webOS 24.' }] };
