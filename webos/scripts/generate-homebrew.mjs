...
if (!/^[\w-]+\/[\w.-]+$/.test(repo ?? '') || !/^[\w.-]+$/.test(tag ?? '')) throw Error('Usage: node scripts/generate-homebrew.mjs owner/repo webos-v0.15.2');
...
const manifest = {
  ...
  appDescription: 'Legendas com Netflix Sans Medium, branco e sombra curta, como padrão do aplicativo.',
...
const index = { paging: { page: 1, count: 1, maxPage: 1, itemsTotal: 1 }, packages: [{ id: app.id, title: app.title, iconUri: manifest.iconUri, manifest, pool: 'main', shortDescription: 'Estilo padrão de legendas com Netflix Sans Medium para LG webOS 24.' }] };
