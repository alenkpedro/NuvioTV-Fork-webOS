...
if (!/^[\w-]+\/[\w.-]+$/.test(repo ?? '') || !/^[\w.-]+$/.test(tag ?? '')) throw Error('Usage: node scripts/generate-homebrew.mjs owner/repo webos-v0.15.0');
...
const manifest = {
  ...
  appDescription: 'Episódios e fontes dentro do player, sete modos de imagem e legendas Netflix Sans.',
...
const index = { paging: { page: 1, count: 1, maxPage: 1, itemsTotal: 1 }, packages: [{ id: app.id, title: app.title, iconUri: manifest.iconUri, manifest, pool: 'main', shortDescription: 'Painel de episódios e proporção da imagem para LG webOS 24.' }] };
