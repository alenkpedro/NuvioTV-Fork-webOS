...
if (!/^[\w-]+\/[\w.-]+$/.test(repo ?? '') || !/^[\w.-]+$/.test(tag ?? '')) throw Error('Usage: node scripts/generate-homebrew.mjs owner/repo webos-v0.8.0');
...
const manifest = {
  ...
  appDescription: 'Progresso, assistidos e favoritos sincronizados com Nuvio, fila offline por perfil e conflitos.',
...
const index = { paging: { page: 1, count: 1, maxPage: 1, itemsTotal: 1 }, packages: [{ id: app.id, title: app.title, iconUri: manifest.iconUri, manifest, pool: 'main', shortDescription: 'Sincronização Nuvio por perfil e player para LG webOS 24.' }] };
