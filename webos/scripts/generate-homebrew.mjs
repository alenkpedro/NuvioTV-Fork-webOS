...
if (!/^[\w-]+\/[\w.-]+$/.test(repo ?? '') || !/^[\w.-]+$/.test(tag ?? '')) throw Error('Usage: node scripts/generate-homebrew.mjs owner/repo webos-v0.16.0');
...
const manifest = {
  ...
  appDescription: 'Player com barra do fork, ícones expansíveis, Reiniciar, busca acelerada e fontes dentro do vídeo.',
...
const index = { paging: { page: 1, count: 1, maxPage: 1, itemsTotal: 1 }, packages: [{ id: app.id, title: app.title, iconUri: manifest.iconUri, manifest, pool: 'main', shortDescription: 'Barra do player baseada no fork e painel de fontes para LG webOS 24.' }] };
