...
if (!/^[\w-]+\/[\w.-]+$/.test(repo ?? '') || !/^[\w.-]+$/.test(tag ?? '')) throw Error('Usage: node scripts/generate-homebrew.mjs owner/repo webos-v0.4.0');
...
const manifest = {
  ...
  appDescription: 'Menu moderno, opções de layout e detalhes com cartões de episódios. Login Nuvio preservado.',
...
const index = { paging: { page: 1, count: 1, maxPage: 1, itemsTotal: 1 }, packages: [{ id: app.id, title: app.title, iconUri: manifest.iconUri, manifest, pool: 'main', shortDescription: 'Layout Modern, detalhes e login Nuvio para LG webOS 24.' }] };
