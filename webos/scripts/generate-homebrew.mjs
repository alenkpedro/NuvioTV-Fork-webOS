...
if (!/^[\w-]+\/[\w.-]+$/.test(repo ?? '') || !/^[\w.-]+$/.test(tag ?? '')) throw Error('Usage: node scripts/generate-homebrew.mjs owner/repo webos-v0.11.0');
...
const manifest = {
  ...
  appDescription: 'Coleções de filmes do TMDB e avaliações de múltiplas fontes via MDBList, com seleção de provedores.',
...
const index = { paging: { page: 1, count: 1, maxPage: 1, itemsTotal: 1 }, packages: [{ id: app.id, title: app.title, iconUri: manifest.iconUri, manifest, pool: 'main', shortDescription: 'Coleções e avaliações para LG webOS 24.' }] };
