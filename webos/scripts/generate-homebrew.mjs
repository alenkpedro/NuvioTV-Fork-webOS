...
if (!/^[\w-]+\/[\w.-]+$/.test(repo ?? '') || !/^[\w.-]+$/.test(tag ?? '')) throw Error('Usage: node scripts/generate-homebrew.mjs owner/repo webos-v0.17.0');
...
const manifest = {
  ...
  appDescription: 'Painéis de áudio e legendas do fork, sincronização por fala e ajuste fino de 100 ms.',
...
const index = { paging: { page: 1, count: 1, maxPage: 1, itemsTotal: 1 }, packages: [{ id: app.id, title: app.title, iconUri: manifest.iconUri, manifest, pool: 'main', shortDescription: 'Player do fork com sincronização por fala para LG webOS 24.' }] };
