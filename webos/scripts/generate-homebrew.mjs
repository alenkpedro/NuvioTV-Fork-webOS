...
if (!/^[\w-]+\/[\w.-]+$/.test(repo ?? '') || !/^[\w.-]+$/.test(tag ?? '')) throw Error('Usage: node scripts/generate-homebrew.mjs owner/repo webos-v0.10.0');
...
const manifest = {
  ...
  appDescription: 'Elenco dos addons, perfis de pessoas, filmografia, recomendações TMDB e trailers no YouTube.',
...
const index = { paging: { page: 1, count: 1, maxPage: 1, itemsTotal: 1 }, packages: [{ id: app.id, title: app.title, iconUri: manifest.iconUri, manifest, pool: 'main', shortDescription: 'Detalhes, filmografia e trailers para LG webOS 24.' }] };
