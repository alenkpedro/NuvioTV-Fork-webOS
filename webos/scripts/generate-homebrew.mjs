...
if (!/^[\w-]+\/[\w.-]+$/.test(repo ?? '') || !/^[\w.-]+$/.test(tag ?? '')) throw Error('Usage: node scripts/generate-homebrew.mjs owner/repo webos-v0.2.0');
...
const manifest = {
  ...
  appDescription: 'Port para LG UT8050: revisão visual baseada no fork. Esta atualização ainda requer validação na TV.',
...
const index = { paging: { page: 1, count: 1, maxPage: 1, itemsTotal: 1 }, packages: [{ id: app.id, title: app.title, iconUri: manifest.iconUri, manifest, pool: 'main', shortDescription: 'Layout do fork, biblioteca local, addons e player HTTP(S) para LG webOS 24.' }] };
