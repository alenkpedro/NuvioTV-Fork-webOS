...
4. **Descoberta e metadados — primeira entrega 0.9.** Descobrir com filtros,
   ...
   **Quarta rodada na TV 0.28:** **telas de carregamento do fork** (abertura com wordmark e anel, esqueleto dos detalhes e das fontes, esqueleto por fileira na Home), **Home progressiva** (cada fileira entra quando o seu add-on responde, sem esperar o mais lento) e **cache de catálogos e metadados de 5 minutos**; as **coleções na Home mostram a capa de cada pasta** (imagem ou emoji, com o formato do `tileShape`), com capas e ajustes de aparência preservados no blob da conta; as **miniaturas não guardam mais quadro preto** (captura no quadro apresentado, aviso único quando a TV não entrega o quadro e prévia sempre com o quadro mais próximo); a **informação de reprodução vai para o canto** e a linha de ícones acima da linha do tempo não muda de lugar; o **primeiro nome do elenco** na pausa e a **primeira cor** da legenda não são mais cortados. Ver [LOADING.md](LOADING.md), [PLAYER_EXTRAS.md](PLAYER_EXTRAS.md) e [COLLECTIONS.md](COLLECTIONS.md).
