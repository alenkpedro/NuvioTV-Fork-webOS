...
test('profile PIN gates account data, inherits primary addons and separates favorites across reload',async({page})=>{
  ...
  expect(calls.some(c=>/sync_push|sync_delete|set_profile_pin/.test(c.path))).toBe(false);expect(await page.evaluate(()=>{const containsPIN=value=>value==='1234' || value===1234 || Boolean(value && typeof value==='object' && Object.values(value).some(containsPIN));return containsPIN(JSON.parse(localStorage.getItem('nuvio-fork.webos.v1')));})).toBe(false);expect(errors).toEqual([]);
});
