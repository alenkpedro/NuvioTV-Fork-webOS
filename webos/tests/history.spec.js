...
test('remote progress resumes native video at imported seconds and source fallback is explicit',async({page})=>{
 ...
 for(let i=0;i<5 && await page.locator('video').count();i++)await page.keyboard.press('Escape');await page.keyboard.press('Escape');await history(page);await expect(page.locator('.history-summary')).toContainText('Trakt está selecionado no Android');await expect(page.locator('.history-summary')).toContainText('usando Nuvio Sync');expect(calls.some(c=>/provider_credentials|trakt|simkl/.test(c.path))).toBe(false);expect(errors).toEqual([]);
});
